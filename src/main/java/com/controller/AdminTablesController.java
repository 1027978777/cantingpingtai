...
public class AdminTablesController  {
   ...
   @RequestMapping("search")
    public String search(HttpServletRequest request,String name){
        String key = "%" + name + "%";
        List<Tables> list = tablesService.search(key);
     ...
     @RequestMapping("add")
    public String add(HttpServletRequest request,Tables model){
    ...
    public String edit(HttpServletRequest request,Integer id){
       Tables model = tablesService.getModel(id);
       request.setAttribute("model",model);
        return "/admin/tables-add";
    }
    ...
    @RequestMapping("save")
    @ResponseBody
    public Result save(HttpServletRequest request,Tables model){
        int i = 0;
        if (model.getId() == null) {
            i = tablesService.insertModelWithoutNull(model);
        } else {
            i = tablesService.updateModelWithoutNull(model);
        }
        if (i == 0) {
    ...
    public Result delete(HttpServletRequest request,Integer id){
        int i = tablesService.deleteModel(id);
        if (i == 0) {
            return ResultUtil.error("失败");
        } else {
            return ResultUtil.success(null);
   ...
   @RequestMapping("list")
    public String list(HttpServletRequest request,Tables model){
        List<Tables> list = tablesService.getModelList(putEntityInMap(model));
        request.setAttribute("list",list);
        request.setAttribute("model",model);
        return "/admin/tables-list";
    }
     ...
     **/
    public Map<String,Object> putEntityInMap(Tables entity){
