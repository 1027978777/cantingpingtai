...
<body class="layui-anim layui-anim-up">
   ...
   <xblock>
        ...
        <button class="layui-btn" onclick="x_admin_show('添加','/admin/tables/add?bid=${model.bid}')"><i class="layui-icon"></i>添加</button>
      ...
      <table class="layui-table">
        ...
        <tbody>
         ...
         <tr>
            ...
            <td>
                <c:if test="${model.state eq 0}">空闲</c:if>
                <c:if test="${model.state eq 1}">已使用</c:if>
                <c:if test="${model.state eq 2}">故障</c:if>
            </td>
           ...
           <td class="td-manage">
                 ...
                 <button class="layui-btn layui-btn layui-btn-xs layui-bg-orange"
                        onclick="x_admin_show('编辑','/admin/tables/edit?id=${model.id}')"><i
    ...
    <script>
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？删除后无法恢复',function(index){
              //发异步删除数据
              $.post("/admin/tables/delete",{id:id},function(result){
                  if (result.code == 200){
        ...
        layer.confirm('确认要删除吗？',function(index){
            ...
            for (i =0;i<ids.length; i++){
                 $.post("/admin/tables/delete",{id:ids[i]},function(result){
