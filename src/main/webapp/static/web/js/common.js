var funParabola = function (d, t, g) {
    var i = {
        speed: 166.67, curvature: 0.001, progress: function () {
        }, complete: function () {
        }
    };
    var p = {};
...
function startMove(e, b, c, a, d) {
    ...
    e.timer = setInterval(function () {
        ...
        for (var f in b) {
            var i = 0;
            if (f == "opacity") {
                i = parseInt(parseFloat(getStyle(e, f)) * 100)
            } else {
                if (a) {
                    i = parseInt(e[f])
                } else {
                    i = parseInt(getStyle(e, f))
                }
            }
            if (isNaN(i)) {
                i = 0
            ...
            if (f == "opacity") {
                e.style.filter = "alpha(opacity:" + (i + h) + ")";
                e.style.opacity = (i + h) / 100
            } else {
                if (a) {
                    e[f] = i + h
                } else {
        ...
        return {
            ...
            link: function (scope, elem, attrs) {
                ...
                function slideLeft(idx, lastIdx) {
                    if (idx == lastIdx) return;
                    var self = this;
                    startMove(self.img[lastIdx], {"left": "-" + self.width}, function () {
                        img[lastIdx].style.left = self.width + "px";
                        status = 0;
                    }, "", 10);
                ...
                function slideTop(idx, lastIdx) {
                    if (idx == lastIdx) return;
                    var self = this;
                    startMove(self.img[lastIdx], {"top": "-" + self.height}, function () {
                        img[lastIdx].style.top = self.height + "px";
                    });
...
var loginCtrl = ["$scope", "formVaildate", "$http", function (scope, fem, $http) {
    ...
    scope.loginVaildate = function () {
        ...
        $.post("/web/user/login",{username:scope.user.username,password:scope.user.password},function (result) {
           if (result.code == 200){
               scope.user.usernameMessage = '登陆成功！';
               location.reload();
           } else {
               scope.user.usernameMessage = '账号密码错误';
           }
        })
    ...
    scope.feedbackSubmit = function () {
        ...
        if (vaildate) {
            $http.post(feedbackUrl, {
                "username": scope.feedback.userContact,
                'message': scope.feedback.feedbackMessage
            }).success(function (d) {
                childScope.userFeedback = false;
            }).error(function () {
                childScope.userFeedback = false;
